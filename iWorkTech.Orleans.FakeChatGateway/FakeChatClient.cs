...
{
    public class FakeChatClient : IDisposable
    {
        ...
        public FakeChatClient(bool synchronousCallbacks = false, IHubProtocol protocol = null,
            IInvocationBinder invocationBinder = null, bool addClaimId = false)
        {
            ...
            Application = ChannelConnection.Create<byte[]>(applicationToTransport, transportToApplication);
            _transport = ChannelConnection.Create<byte[]>(transportToApplication, applicationToTransport);
        ...
        public async Task<HubMessage> ReadAsync()
        {
            while (true)
            {
                ...
                if (message == null)
                {
                    if (!await Application.In.WaitToReadAsync()) return null;
                }
                else
                {
                    return message;
